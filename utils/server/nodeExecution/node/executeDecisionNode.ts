import { FlowNode, DecisionNodeData, DecisionBranch, ConditionGroup, DecisionCondition } from '../../../../models/flowTypes';
...
function getInputFromBranch(branchs: DecisionBranch[]): string[] {
    ...
    const inputs: string[] = [];
    branchs.forEach(b => {
        if (b.groups) b.groups.forEach(g => {
            if (g.conditions) g.conditions.forEach(c => {
...
function evaluateCondition(condition: DecisionCondition, inputs: Record<string, any>): boolean {
    ...
    switch (condition.operator) {
        ...
        case 'notEquals': return val1 !== val2;
        case 'contains': return val1.includes(val2);
        case 'greaterThan': return parseFloat(val1) > parseFloat(val2); // Ensure numeric comparison
        case 'lessThan': return parseFloat(val1) < parseFloat(val2); // Ensure numeric comparison
        case 'startsWith': return val1.startsWith(val2);
        case 'endsWith': return val1.endsWith(val2);
        default:
...
export async function executeDecisionNode(node: FlowNode, { flowState }: FlowExecutionContext): Promise<ExecutionResult> {
    ...
    if (!ready) {
        return {
            nextNodes: [],
            status: 'waiting',
            message: `Waiting for inputs for decision node '${nodeLabel}'`,
            flowState,
            nodeInfo: { id: node.id, name: nodeLabel, type: 'decision', role: 'developer' },
            execution: { output: 'Waiting', nodeId: node.id, nodeName: nodeLabel, startTime: new Date().toISOString() },
        };
    }
