...
const ChatMessage: React.FC<ChatMessageProps> = ({ message }) => {
  ...
  const roleLabel =
    sender === 'user' ? 'You' : sender === 'system' ? 'System' : sender === 'developer' ? 'Developer' : 'Agent';
        ...
        <Space direction="vertical" size={isMobile ? 'small' : 'middle'} style={{ width: '100%' }}>
          {sender !== 'developer' && (
            <Space size={isMobile ? 'small' : 'middle'}>
              ...
              <Typography.Text
                type="secondary"
                style={{ fontSize: isMobile ? token.fontSizeSM - 1 : token.fontSizeSM }}>
                {roleLabel}
              </Typography.Text>
              <Typography.Text
                type="secondary"
              ...
              {executionStatus && (
                <Tag
                  color={executionStatus.nodeType === 'interface' ? 'blue' : 'default'}
                  style={{ fontSize: isMobile ? token.fontSizeSM - 1 : undefined }}>
          ...
          <Typography.Paragraph
            ...
            copyable>
            {sender === 'developer' ? (
              <Typography.Text type="secondary">{`[${formattedTime}] ${text.slice(-50)}`}</Typography.Text>
            ) : (
              <ReactMarkdown>{text}</ReactMarkdown>
            )}
          </Typography.Paragraph>
