...
import  { useNodesState, useEdgesState ,  ReactFlow,
} from "@xyflow/react";
...
const FlowWithExecution = () => {
    ...
    }, [edges, result]); // Rebuild graph when edges or result changes
    ...
    const buildDependencyGraph = () => {
        ...
        edges.forEach((edge) => {
            ...
            if (edge.data?.condition) {
                ...
                try {
                    if (eval(condition)) {  // CAUTION: eval() can be dangerous. Use with caution.
    ...
    const executeNodeLogic = async (nodeId: string) => {
        ...
        setNodes((nds) =>
            nds.map((node) =>
                node.id === nodeId
                    ? { ...node, style: { ...node.style, backgroundColor: 'lightgreen' } }
                    : node
            )
        );
    };
    ...
    const depthFirstSearch = async (nodeId: string, visited = new Set<string>()) => {
        ...
        setNodes((nds) =>
            nds.map((node) =>
                node.id === nodeId
                    ? { ...node, style: { ...node.style, backgroundColor: 'lightblue' } }
                    : node
            )
        );
    ...
    const handleExecuteFlow = async () => {
        ...
        setNodes((nds) =>
            nds.map((node) => ({ ...node, style: { ...node.style, backgroundColor: 'white' } }))
        );
