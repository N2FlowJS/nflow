import { AppstoreOutlined, ArrowRightOutlined, CloudOutlined, DatabaseOutlined, FileOutlined, LoginOutlined, RobotOutlined, UserAddOutlined, UserOutlined } from '@ant-design/icons';
import { Avatar, Button, Card, Col, List, Row, Space, Statistic, Tag, Typography } from 'antd';
...
export default function Home() {
  ...
  const loadData = async () => {
    ...
    try {
      const [filesData, knowledgeData, agentsCount] = await Promise.all([fetchAllFiles(), fetchAllKnowledge(), getAgentCount()]);
      setFiles(filesData);
          ...
          <>
            ...
            <Row gutter={[24, 32]} align="middle" style={{ marginBottom: 48 }}>
              <Col xs={24} md={14}>
                <div style={{ padding: '20px 0' }}>
                  ...
                  <Paragraph style={{ fontSize: 18, marginBottom: 24 }}>{messages.home.heroDescription}</Paragraph>

                  {!isAuthenticated ? (
                    <Space size="middle">
                      <Button type="primary" size="large" icon={<LoginOutlined />} onClick={() => router.push('/auth/login')}>
                        {messages.home.signIn}
                      </Button>
                      <Button size="large" icon={<UserAddOutlined />} onClick={() => router.push('/auth/register')}>
                        {messages.home.createAccount}
                      </Button>
                    </Space>
                  ) : (
                    <Button type="primary" size="large" icon={<AppstoreOutlined />} onClick={() => router.push('/dashboard')}>
                ...
                <Card>
                  {isAuthenticated ? (
                    <div style={{ display: 'flex', alignItems: 'center' }}>
                      ...
                      <div style={{ marginLeft: 16 }}>
                        <Title level={3} style={{ marginBottom: 4 }}>
                          <Button type="link" style={{ padding: 0 }} onClick={() => router.push(`/user/${user?.id}`)}>
                            {user?.name}
                          </Button>
                        </Title>
                    ...
                    <div style={{ textAlign: 'center', padding: '20px 0' }}>
                      ...
                      <Paragraph>{messages.home.cloudBasedDescription}</Paragraph>
                    </div>
                  )}
                </Card>
              </Col>
              ...
              <Row gutter={[16, 16]} style={{ marginBottom: 48 }}>
                <Col xs={24} sm={8}>
                  <Card hoverable loading={statsLoading}>
                    <Statistic title={messages.home.knowledgeBases} value={knowledge.length} prefix={<DatabaseOutlined />} valueStyle={{ color: '#1677ff' }} />
                ...
                <Col xs={24} sm={8}>
                  <Card hoverable loading={statsLoading}>
                    <Statistic title={messages.home.files} value={files.length} prefix={<FileOutlined />} valueStyle={{ color: '#52c41a' }} />
                    <div style={{ marginTop: 16 }}>
                ...
                <Col xs={24} sm={8}>
                  <Card hoverable loading={statsLoading}>
                    <Statistic title={messages.home.aiAgents} value={agentCount} prefix={<RobotOutlined />} valueStyle={{ color: '#722ed1' }} />
                    <div style={{ marginTop: 16 }}>
                ...
                {features.map((feature, index) => (
                  <Col xs={24} md={8} key={index}>
                    <Card
                      ...
                      style={{ height: '100%' }}
                      actions={[
                        <Button type="link" key="explore" onClick={feature.action}>
            ...
            {isAuthenticated && files.length > 0 ? (
                  ...
                  <List
                    ...
                    renderItem={(file) => (
                      <List.Item
                        key={file.id}
                        actions={[
                          <Button type="link" key="view" onClick={() => router.push(`/files/${file.id}`)}>
                            {messages.home.view}
                          </Button>,
                        ]}>
                        <List.Item.Meta avatar={<FileOutlined style={{ fontSize: 20 }} />} title={file.originalName} description={`Uploaded: ${new Date(file.createdAt).toLocaleString()}`} />
                        <div>
                          <Tag color={file.parsingStatus === 'completed' ? 'green' : 'orange'}>{file.parsingStatus || 'pending'}</Tag>
                        </div>
                      </List.Item>
                    )}
                  />
              ...
              <Card style={{ textAlign: 'center', padding: '24px' }}>
                ...
                <Paragraph style={{ fontSize: 16, marginBottom: 24 }}>{messages.home.getStartedDescription}</Paragraph>
                <Steps
...
function Steps({ isAuthenticated, onLoginClick, onRegisterClick, onKnowledgeClick, onFileClick, onAgentClick, messages }: StepsProps) {
  return (
    <List
      bordered
      dataSource={[
        {
          ...
          description: isAuthenticated ? messages.home.stepsLoggedInDescription : messages.home.stepsCreateAccountDescription,
        ...
        },
        {
          ...
          action: (
            <Button type="primary" onClick={onKnowledgeClick} disabled={!isAuthenticated}>
        ...
        },
        {
          ...
          action: (
            <Button type="primary" onClick={onFileClick} disabled={!isAuthenticated}>
        ...
        },
        {
          ...
          action: (
            <Button type="primary" onClick={onAgentClick} disabled={!isAuthenticated}>
