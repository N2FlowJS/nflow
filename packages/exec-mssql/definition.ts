...
/**
 * MSSQL Execute Node Definition
 * 
 * Execute T-SQL queries on Microsoft SQL Server databases.
 ...
 * - query: T-SQL query (supports {variable} templates)
 * - trustServerCertificate: Trust server certificate (for dev)
 ...
 * ```
 */
export const ExecMssqlNodeDefinition: NodeDefinition = {
  id: 'exec-mssql',
  name: 'MSSQL Execute',
  category: NodeCategory.DATABASE,
  description: 'Execute T-SQL queries on Microsoft SQL Server databases',
  version: '1.0.0',
  ...
  inputs: [
    ...
    },
    {
      ...
      required: true,
      metadata: { inputType: 'text', placeholder: 'master' },
    },
    {
      id: 'user',
      name: 'User',
      ...
      required: true,
      metadata: { inputType: 'text', placeholder: 'sa' },
    },
    {
      id: 'password',
      name: 'Password',
    ...
    },
    {
      ...
      name: 'T-SQL Query',
      type: PortType.TEXT,
      description: 'T-SQL query (supports {variable} templates)',
      required: true,
      metadata: { inputType: 'textarea', rows: 6, placeholder: 'SELECT * FROM Users WHERE Status = {status}' },
    },
    {
      id: 'trustServerCertificate',
      name: 'Trust Server Certificate',
      type: PortType.BOOLEAN,
      description: 'Trust server certificate (use for development)',
      required: false,
      defaultValue: false,
  ...
  getDynamicInputs: (config) => {
    ...
    return [...ExecMssqlNodeDefinition.inputs, ...dynamicPorts];
  },
  ...
  async execute(context: NodeExecutionContext): Promise<NodeExecutionResult> {
    ...
    try {
      ...
      const poolConfig = {
        ...
        requestTimeout: ((config.timeout as number) || 30) * 1000,
      };
